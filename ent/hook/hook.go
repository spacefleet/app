...
func (f CLIAuthCodeFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CLIAuthCodeMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.CLIAuthCodeMutation", m)
}

// The CLITokenFunc type is an adapter to allow the use of ordinary
// function as CLIToken mutator.
type CLITokenFunc func(context.Context, *ent.CLITokenMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f CLITokenFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.CLITokenMutation); ok {
		return f(ctx, mv)
	}
